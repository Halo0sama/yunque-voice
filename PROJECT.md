...
- 思考模式（实测）：deepseek `thinking.type=disabled` 可关；qwen `enable_thinking=false` 可关；智谱常思考仅 depth 分档（low/high/max）→ 实时路径统一"关或最浅"；**夜间压缩 thinkingMax=depth:max**：deepseek 与智谱同构支持 depth（实测），qwen 显式开启（质量优先）
